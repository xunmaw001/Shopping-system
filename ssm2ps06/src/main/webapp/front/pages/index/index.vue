<template>
	<view class="uni-padding-wrap">
				...
				<swiper class="swiper" :indicator-dots='".swiper-pagination"==null?false:true' :autoplay='autoplaySwiper' :circular='true' indicator-active-color='rgba(45, 234, 234, 1)' indicator-color='rgba(0, 0, 0, .3)' :duration='1000' :interval='intervalSwiper' :vertical='"vertical"=="horizontal"?false:true'>
					<swiper-item v-for="(swiper,index) in swiperList" :key="index" @tap="onSwiperTap(swiper)">
						<image mode="aspectFill" :src="swiper.img"></image>
										...
										<!--header end-->
		<view class="pl15 col3" style="padding: 0 12px;box-sizing: border-box;" :style='{"backgroundColor":"rgba(74, 209, 242, 1)","borderColor":"rgba(132, 122, 122, 1)","color":"rgba(255, 255, 255, 0.64)","borderRadius":0,"textAlign":"center","borderWidth":"2rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"solid"}'>商品信息推荐</view>
		<view v-if='2 == 1 || 2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('shangpinxinxi',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in shangpinxinxilist" :key="index">
				                				<view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.shangpinmingcheng}}</view>
								                				                				<view class="image-view" :style='{"borderRadius":"20rpx","height":"320rpx"}'>
					<image mode="aspectFill" :style='{"borderRadius":"20rpx","height":"320rpx"}' class="uni-product-image" :src="product.tupian?product.tupian.split(',')[0]:''"></image>
				</view>
		...
		<view class="list" v-if='2 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('shangpinxinxi',product.id)" v-for="(product,index) in shangpinxinxilist" :key="index" class="listm flex flex-between">
																<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.shangpinmingcheng}}</view>
				</view>
																																												<image mode="aspectFill" :style='{"borderRadius":"20rpx","height":"320rpx"}' class="listmpic" :src="product.tupian?product.tupian.split(',')[0]:''"></image>
																																																																																																																																																																																<view class="listmr">
		...
		<!--list end-->
			...
			<view class="right-content" @tap="onPageTap('tejiashangpin')" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='1 == 1 || 1 == 2' class="uni-product-list" :class="1 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('tejiashangpin',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in hometejiashangpinlist" :key="index">
				                				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}' class="uni-product-title">{{product.shangpinmingcheng}}</view>
								                				                				<view class="image-view" :style='{"borderRadius":"20rpx","height":"320rpx"}'>
					<image :style='{"borderRadius":"20rpx","height":"320rpx"}' mode="aspectFill" class="uni-product-image" :src="product.tupian?product.tupian.split(',')[0]:''"></image>
                				                				                				                				                				                				                				                				...
                				                				                				                				                				                				                				                				<view class="uni-product-price" :style='{"fontSize":"24rpx","lineHeight":"48rpx","color":"rgba(11, 11, 11, 1)","textAlign":"center"}'>
		...
		<view class="list" v-if='1 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('tejiashangpin',product.id)" v-for="(product,index) in hometejiashangpinlist" :key="index" class="listm flex flex-between">
																<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.shangpinmingcheng}}</view>
				</view>
																																												<image :style='{"borderRadius":"20rpx","height":"320rpx"}' mode="aspectFill" class="listmpic" :src="product.tupian?product.tupian.split(',')[0]:''"></image>
																																																																																																																																																												<view class="listmr">
					<view v-if="item.addtime" class="colb f24 lh35" :style='{"fontSize":"24rpx","lineHeight":"48rpx","color":"rgba(11, 11, 11, 1)","textAlign":"center"}'>￥{{product.price}}</view>
				</view>
															</view>
		</view>
		<!--list end-->
	...
	export default {
		...
		async onShow() {
			...
			this.shangpinxinxilist = res.data.list
																					...
																					res = await this.$api.list('tejiashangpin', {
				page: 1,
				limit: 4
			});
			this.hometejiashangpinlist = res.data.list
																																																																				},
		methods: {
			//轮播图跳转
			onSwiperTap(e) {
