	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view v-if='1 == 3' class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(160, 156, 156, 0.82)"}'>
			<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
																																																<image :style='{"borderRadius":"8rpx","width":"100%","height":"100%"}' mode="aspectFill" class="listmpic" :src="product.touxiang?product.touxiang.split(',')[0]:''"></image>
																																												<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.yonghuming}}</view>
																																																																																									</view>
																																																																							</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				let res = await this.$api.list(`yonghu`, {
					page: mescroll.num,
					limit: mescroll.size
				});
				// 如果是第一页数据置空
